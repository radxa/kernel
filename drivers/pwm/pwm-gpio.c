...
static enum hrtimer_restart gpio_pwm_timer(struct hrtimer *timer)
{
	...
	enum pwm_polarity polarity = pwm->state.polarity;

	if (!pwm->state.enabled) {
		gpio_pwm_off(pc, polarity);
		pc->pin_on = false;
			   ...
			   const struct pwm_state *state)
{
	...
	if (state->polarity != pwm->state.polarity) {
		/*
		 * Polarity changed while running.  The timer handles polarity
		 * on each toggle, so no explicit action needed — but we must
		 * restart the timer if currently running, so the next toggle
		 * picks up the new polarity immediately.
		 */
	}
	...
	if (!state->enabled) {
		if (was_enabled) {
			...
			gpio_pwm_off(pc, state->polarity);
			pc->pin_on = false;
		}
		return 0;
	}
	...
	if (!was_enabled) {
		if (pc->off_time) {
			hrtimer_start(&pc->timer, ktime_set(0, 0),
				      HRTIMER_MODE_REL);
		} else {
			if (pc->on_time)
				gpio_pwm_on(pc, state->polarity);
			else
				gpio_pwm_off(pc, state->polarity);
		}
	}
